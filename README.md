# 🏗️ K8s-LLM-Inference-Platform

> Kubernetes 云原生 AI 推理服务运维平台

[![License](https://img.shields.io/badge/license-MIT-blue.svg)](https://opensource.org/licenses/MIT)
[![Kubernetes](https://img.shields.io/badge/Kubernetes-K3s-orange)](https://kubernetes.io/)
[![vLLM](https://img.shields.io/badge/AI-vLLM-purple)](https://vllm.ai/)

## 📖 项目简介

本项目提供了一套完整的 **Kubernetes 云原生 AI 推理服务运维平台** 解决方案，基于 K3s + vLLM + Prometheus + Grafana 技术栈，实现了大语言模型推理服务的一键部署、监控告警和弹性伸缩。

## 🏗️ 技术架构

```
┌─────────────────────────────────────────────────────────────────────┐
│                           用户访问层                                  │
│                    (Traefik / Nginx Ingress)                        │
└─────────────────────────────────────────────────────────────────────┘
                                    │
                                    ▼
┌─────────────────────────────────────────────────────────────────────┐
│                        K8s 集群层 (K3s)                            │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │                    LLM Serving                               │   │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐        │   │
│  │  │ vLLM Pod    │  │ vLLM Pod    │  │ vLLM Pod    │        │   │
│  │  │ (Qwen2.5-7B)│  │ (Qwen2.5-7B)│  │ (Qwen2.5-7B)│        │   │
│  │  └─────────────┘  └─────────────┘  └─────────────┘        │   │
│  └─────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────┘
                                    │
        ┌───────────────────────────┼───────────────────────────┐
        ▼                           ▼                           ▼
┌───────────────┐          ┌──────────────────┐        ┌─────────────┐
│  监控层       │          │  存储层          │        │  网络层     │
│               │          │                  │        │             │
│ • Prometheus  │          │ • 本地存储       │        │ • Traefik   │
│ • Grafana    │          │ • PVC           │        │ • Service   │
│ • DCGM Exp   │          │ • ConfigMap     │        │
└──────────── │ • Ingress  ───┘          └──────────────────┘        └─────────────┘
```

## ✨ 核心特性

| 特性 | 描述 |
|------|------|
| **一键部署** | Helm Chart 一键部署 vLLM 推理服务 |
| **GPU 调度** | NVIDIA Device Plugin 支持 GPU 调度 |
| **弹性伸缩** | HPA 自动根据负载扩缩容 |
| **全链路监控** | Prometheus + Grafana + DCGM Exporter |
| **智能告警** | 三级告警 (轻微/严重/紧急) |
| **高可用** | 多副本部署 + 健康检查 |

## 🛠️ 技术栈

- **容器编排**: Kubernetes (K3s / 阿里云 ACK)
- **AI 推理框架**: vLLM (PagedAttention)
- **模型**: Qwen2.5-7B-Instruct
- **监控**: Prometheus + Grafana + node_exporter + DCGM Exporter
- **存储**: Local Path Provisioner
- **Ingress**: Traefik / Nginx Ingress
- **部署工具**: Helm + Ansible

## 📁 项目结构

```
k8s-llm-inference-platform/
├── k8s/                    # Kubernetes YAML 配置
│   ├── namespace.yaml       # 命名空间
│   ├── configmap.yaml       # 配置
│   ├── pvc.yaml           # 持久化存储
│   ├── deployment.yaml    # 部署配置
│   ├── service.yaml       # 服务
│   ├── ingress.yaml       # 入口
│   ├── hpa.yaml          # 自动伸缩
│   └── servicemonitor.yaml # 监控服务
│
├── helm/                   # Helm Chart
│   └── llm-inference/     # vLLM 推理服务 Chart
│
├── docker/                  # Docker 镜像
│   └── Dockerfile
│
├── ansible/                # Ansible 自动化
│   └── playbooks/
│
├── scripts/                # 运维脚本
│   ├── deploy.sh
│   ├── benchmark.sh
│   └── monitor.sh
│
├── docs/                   # 文档
│   └── ...
│
└── demo/                   # 演示文件
    └── ...
```

## 🚀 快速开始

### 1. 环境要求

| 组件 | 最低配置 |
|------|----------|
| Kubernetes | K3s / ACK |
| GPU | NVIDIA T4 / A10 (16GB+ 显存) |
| 内存 | 32GB+ |
| 系统 | Ubuntu 22.04 |

### 2. 部署步骤

```bash
# 1. 安装 K3s
curl -sfL https://get.k3s.io | sh -

# 2. 安装 NVIDIA Device Plugin
kubectl create -f https://raw.githubusercontent.com/NVIDIA/k8s-device-plugin/v0.14.5/nvidia-device-plugin.yml

# 3. 部署 vLLM 推理服务
kubectl apply -f k8s/

# 4. 或使用 Helm 部署
helm install llm-serving helm/llm-inference/

# 5. 部署监控
helm install prometheus prometheus-community/kube-prometheus-stack -n monitoring
```

### 3. 验证部署

```bash
# 查看 Pod 状态
kubectl get pods -n llm-serving

# 测试 API
curl http://<your-ip>:8000/v1/models

# 查看监控面板
kubectl port-forward -n monitoring svc/prometheus-grafana 3000:80
```

## 📊 监控面板

![智算监控中心](智算监控中心.png)

配套监控面板项目：[k8s-llm-monitor](https://github.com/fuqiangfeng96-web/k8s-llm-monitor)

## 🔔 告警规则

| 级别 | 阈值 | 示例 |
|------|------|------|
| 🟡 轻微 | > 70% | CPU/内存使用率偏高 |
| 🟠 严重 | > 85% | GPU 显存紧张、Pod Pending |
| 🔴 紧急 | > 95% | 资源耗尽、Pod Failed |

## 📚 文档

- [实施指导书](./项目1：Kubernetes_云原生_AI_推理服务运维平台_·_实施指导书_305afb83e0458014a612ecb96dd6dc02.md)
- [K8s 部署配置](./k8s/)
- [Helm Chart](./helm/)

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！

## 📄 许可证

MIT License

---

*如果对你有帮助，欢迎 Star ⭐*
